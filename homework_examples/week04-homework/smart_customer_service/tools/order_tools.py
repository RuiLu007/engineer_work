...
from smart_customer_service.order_utils import is_valid_order_id, normalize_order_id
...
def query_order(order_id: str) -> dict:
    ...
    print(f"--- [工具调用] 正在查询订单号: {normalized_order_id} ---")
    time.sleep(2)
    if not is_valid_order_id(normalized_order_id):
        return {
            "success": False,
            "order_id": normalized_order_id,
            "error": "订单号格式无效，应为类似 SN20240924003 的编号。"
        }
    ...
    order_info = mock_db.get(normalized_order_id)
    if order_info:
        return {
            "success": True,
            "order_id": normalized_order_id,
...
def apply_refund(order_id: str, reason: str) -> dict:
    ...
    """
    normalized_order_id = normalize_order_id(order_id)
    print(f"--- [工具调用] 正在为订单号 {normalized_order_id} 申请退款，原因: {reason} ---")
    time.sleep(1)
    if is_valid_order_id(normalized_order_id):
...
def generate_invoice(order_id: str) -> dict:
    ...
    """
    normalized_order_id = normalize_order_id(order_id)
    print(f"--- [工具调用] 正在为订单号 {normalized_order_id} 生成发票 ---")
    time.sleep(1)
    if is_valid_order_id(normalized_order_id):
        invoice_url = f"https://example.com/invoices/{normalized_order_id}.pdf"
        return {
            "success": True,
            "order_id": normalized_order_id,
