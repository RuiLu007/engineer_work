...
def cached_multi_hop_query(question: str):
    """缓存多跳查询结果"""
...
def _initialize_rag_index():
    ...
    try:
        if not os.path.exists(config.INDEX_DIR):
            logger.info("未找到向量索引，正在从文件创建...")
            documents = SimpleDirectoryReader(
                input_files=[config.COMPANY_DOC_PATH]
            ).load_data()
            index = VectorStoreIndex.from_documents(documents)
            index.storage_context.persist(persist_dir=config.INDEX_DIR)
            logger.info(f"向量索引已创建并保存在 '{config.INDEX_DIR}'。")
        else:
            logger.info(f"从 '{config.INDEX_DIR}' 加载现有向量索引...")
        ...
        logger.error(f"初始化RAG索引时出错: {str(e)}")
        raise
...
def _initialize_kg_query_engine():
    ...
    try:
        logger.info("正在连接到 Neo4j 并初始化知识图谱查询引擎...")
        ...
        logger.info("知识图谱查询引擎初始化成功。")
    except Exception as e:
        logger.error(f"初始化知识图谱查询引擎时出错: {str(e)}")
        raise
    ...
    try:
        ...
        if "最大股东" in question or "控股" in question:
            ...
            try:
                ...
                logger.error(f"执行图谱查询时出错: {str(e)}")
                kg_result_text = f"查询出错: {str(e)}"
        ...
        try:
            ...
            logger.error(f"执行RAG查询时出错: {str(e)}")
            rag_context = f"检索出错: {str(e)}"
            reasoning_path.append(f"步骤 3: RAG 检索失败: {str(e)}")
        ...
        logger.error(f"执行多跳查询时出错: {str(e)}")
        raise
...
def initialize_all():
    try:
        ...
        logger.info("所有查询引擎初始化完成。")
    except Exception as e:
        logger.error(f"初始化查询引擎时出错: {str(e)}")
        # 不中断程序启动，允许后续手动初始化

# --- 重新初始化引擎 ---
def reinitialize_engines():
    """重新初始化所有引擎"""
    global _rag_query_engine, _kg_query_engine
    _rag_query_engine = None
    _kg_query_engine = None
    # 清除缓存
    cached_multi_hop_query.cache_clear()
    initialize_all()
